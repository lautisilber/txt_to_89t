...
class TI_TextWriter(TI_Writer):
    ...
    def write_content(self, stream: typing.BinaryIO) -> None:
        ...
        for c in self.text_content:
            code = ord(c)

            if code > 127:
                code = cls.unicode_to_ti(c)
            elif c == "\n":
                code = ord("\r")
...
if __name__ == "__main__":
    writer = TI_TextWriter(
        ...
        folder_name="main",
        variable_name="a_test",
        comment="generated by txt_to_89t",
        text_content="This is come content that I want to\ninclude in\nmy file",
    )

    with open("test/output.89t", "wb") as f:
        writer.write(f)
