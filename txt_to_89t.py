...
class TI_Writer(abc.ABC):
    ...
    def __post_init__(self) -> None:
        ...
        var_sizes = (
            VarSize("folder_name", 8),
            VarSize("variable_name", 8),
            VarSize("comment", 40),
        )
    ...
    def write_checksum(self, stream: typing.BinaryIO) -> None:
        ...
        cls.write_byte(stream, crcsum)  # hi a low bytes of crc sum
...
class TI_TextWriter(TI_Writer):
    ...
    def write_content(self, stream: typing.BinaryIO) -> None:
        ...
        # cls.write_byte(stream, 0x20)

        self.sum = (length4 & 0xFF) + ((length4 >> 8) & 0xFF)

        for c in map(ord, self.text_content):
            if c > 128:
                c = cls.unicode_to_ti(chr(c))
            elif chr(c) == "\n":
                c = ord("\r")
            self.sum += c
            cls.write_byte(stream, c)
